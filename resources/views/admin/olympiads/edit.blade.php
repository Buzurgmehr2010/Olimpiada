<x-app-layout>
    @slot('title', 'Редактирование — ' . $olympiad->title)
    ...
    <div class="card">
        <div class="card-header">
            <div style="display: flex; align-items: center; gap: 12px;">
                <div class="admin-form-icon"><i class="bi bi-pencil-square"></i></div>
                <div>
                    <h2 style="font-size: 1.1rem; font-weight: 700; color: var(--text-heading);">{{ $olympiad->title }}</h2>
                    <p style="font-size: 0.82rem; color: var(--text-secondary);">ID: {{ $olympiad->id }}</p>
                </div>
            </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.olympiads.update', $olympiad) }}" method="POST">
                @csrf @method('PUT')
                ...
                <div class="admin-form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-input" value="{{ old('title', $olympiad->title) }}" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Описание <span class="required">*</span></label>
                        <textarea name="description" class="form-textarea" required>{{ old('description', $olympiad->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="admin-form-section">
                    ...
                    <div class="admin-form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="country" class="form-input" value="{{ old('country', $olympiad->country) }}" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="level" class="form-select" required>
                                ...
                                @foreach(['Школьный', 'Региональный', 'Республиканский', 'СНГ', 'Международный'] as $level)
                                    <option value="{{ $level }}" {{ old('level', $olympiad->level) == $level ? 'selected' : '' }}>{{ $level }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="admin-form-section">
                    ...
                    <div class="admin-form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="date" class="form-input" value="{{ old('date', $olympiad->date) }}" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Стоимость <span class="required">*</span></label>
                            <input type="text" name="cost" class="form-input" value="{{ old('cost', $olympiad->cost) }}" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="icon" class="form-input" value="{{ old('icon', $olympiad->icon) }}" style="max-width: 300px;">
                        <p class="form-hint">Bootstrap Icons: <code>trophy</code>, <code>globe</code>, <code>people</code></p>
                    </div>
                </div>

                <div class="admin-form-actions">
                    <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Обновить</button>
                    <a href="{{ route('admin.olympiads.index') }}" class="btn btn-secondary"><i class="bi bi-x-lg"></i> Отмена</a>
                </div>
            </form>
        </div>
    </div>
